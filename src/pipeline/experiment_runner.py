...
def run_experiment():
    ...
    # Usar os dados em escala real, sem normalização
    X_real = (
        df[proc.features_to_use].values
        if hasattr(df, "values")
        else df[proc.features_to_use].to_numpy()
    )
    ...
    models_base = BaselineModels(X_real)
    ...
    # --- LOOP HBOS ---
    print("\n" + "-" * 40)
    print("📊 TREINANDO VARIAÇÕES HBOS")
    for n_bins in [10, 20]:
    ...
    gap_seconds = config.get("configuracoes_gerais", {}).get(
    ...
    for iso_name, iso_mask_inlier in iso_masks_registry.items():
        for hbos_name, hbos_mask_inlier in hbos_masks_registry.items():
            ...
            # União (Rigorosa)
            mask_train_uniao = iso_mask_inlier & hbos_mask_inlier
            ...
            if model_uniao is not None:
                model_uniao.save(
                    f"outputs/models_saved/lstm_uniao_{iso_name}_{hbos_name}.h5"
                )
            ...
            if mse_uniao is not None and idx_uniao is not None:
                ...
                df, metrics = optimizer.apply_dynamic_thresholds(
                    df, f"{lstm_name_uniao}_score", lstm_name_uniao
                )
                results_summary.extend(metrics)

            # Interseção (Permissiva)
            mask_train_inter = iso_mask_inlier | hbos_mask_inlier
            ...
            if model_inter is not None:
                model_inter.save(
                    f"outputs/models_saved/lstm_inter_{iso_name}_{hbos_name}.h5"
                )
            ...
            if mse_inter is not None and idx_inter is not None:
                ...
                df, metrics = optimizer.apply_dynamic_thresholds(
                    df, f"{lstm_name_inter}_score", lstm_name_inter
                )
                results_summary.extend(metrics)
    ...
    mse_sujo, idx_sujo, model_sujo = lstm_pipe.train_evaluate(
        "LSTM_Sujo", mask_train=None, epochs=5
    )
    ...
    if lstm_metrics:
        pd.DataFrame(lstm_metrics).to_csv(
            "outputs/metrics/lstm_metrics.csv", index=False
        )
    ...
    def safe_save(dframe, fname):
        if (
            hasattr(dframe, "to_csv")
            and "single_file" in dframe.to_csv.__code__.co_varnames
        ):
    ...
    safe_save(df_hbos, "outputs/metrics/hbos_results.csv")
