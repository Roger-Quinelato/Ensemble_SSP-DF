...
class GRUPipeline:
    ...
    def __init__(
        ...
        Args:
            X_data: Dados de entrada (numpy ou pandas).
        ...
        logger.info(f"\n{'='*60}")
        logger.info(f"GRU AUTOENCODER - INICIALIZAÇÃO")
        logger.info(f"{'='*60}")
        logger.info(f"  Dados: {self.X.shape[0]:,} registros × {self.X.shape[1]} features")
        logger.info(f"  Veículos únicos: {len(np.unique(self.vehicle_ids)):,}")
    ...
    def create_sequences_with_index(self):
        ...
        logger.info(f"  ✓ Sequências geradas: {sequences_generated:,}")
        logger.info(f"    - Rejeitadas (gap > {max_gap_seconds}s): {sequences_rejected_gap:,}")
        logger.info(f"    - Rejeitadas (dados insuficientes): {sequences_rejected_short:,}")
        
        if len(X_seq_list) == 0:
            logger.info(f"  ⚠️  AVISO: Nenhuma sequência válida!")
    ...
    def build_model(self, n_features):
        ...
        # Mostrar arquitetura
        logger.info(f"\n{'='*60}")
        logger.info(f"ARQUITETURA DO MODELO GRU")
        logger.info(f"{'='*60}")
        model.summary()
        logger.info(f"{'='*60}\n")
    ...
    def train_evaluate(self, strategy_name, mask_train=None, epochs=10, batch_size=64):
        ...
        """
        logger.info(f"\n{'='*60}")
        logger.info(f"TREINAMENTO GRU - {strategy_name}")
        logger.info(f"{'='*60}")
        
        # Gerar sequências
        logger.info(f"  → Gerando sequências (Gap Max: {self.max_gap_seconds}s)...")
        X_seq_all, indices_all = self.create_sequences_with_index()
        
        if len(X_seq_all) == 0:
            logger.info(f"  ❌ Nenhuma sequência válida encontrada!")
            logger.info(f"     Verifique: window_size, max_gap_seconds, ou dados")
            return None, None, None
        
        # Filtragem por máscara (treino semi-supervisionado)
        if mask_train is not None:
            logger.info(f"  → Aplicando máscara de treino (semi-supervisionado)...")
            ...
            logger.info(f"    - Total sequências: {len(X_seq_all):,}")
            logger.info(f"    - Sequências treino: {len(X_train):,} ({len(X_train)/len(X_seq_all)*100:.1f}%)")
            
            if len(X_train) == 0:
                logger.info(f"  ❌ Treino vazio após filtro de máscara!")
                return None, None, None
        else:
            X_train = X_seq_all
            logger.info(f"  → Modo não supervisionado: treinando com todas {len(X_train):,} sequências")
        ...
        logger.info(f"    - Média: {mse_sequences.mean():.6f}")
        logger.info(f"    - Mediana: {np.median(mse_sequences):.6f}")
        logger.info(f"    - Std: {mse_sequences.std():.6f}")
        logger.info(f"    - Min: {mse_sequences.min():.6f}")
        logger.info(f"    - Max: {mse_sequences.max():.6f}")
        logger.info(f"    - P95: {np.percentile(mse_sequences, 95):.6f}")
        logger.info(f"    - P99: {np.percentile(mse_sequences, 99):.6f}")
        logger.info(f"{'='*60}\n")
    ...
    def predict_anomalies(self, threshold_percentile=95):
        ...
        logger.info(f"  Anomalias detectadas: {anomaly_flags.sum():,} ({anomaly_flags.sum()/len(anomaly_flags)*100:.2f}%)")
        
        return anomaly_flags, threshold


# Alias para compatibilidade: LSTMPipeline aponta para GRUPipeline
# (GRU é mais eficiente e tem performance equivalente para window_size <= 20)
