...
class LSTMPipeline:
    ...
    Suporta entrada em numpy ou pandas.
    """
    ...
    def train_evaluate(self, strategy_name, mask_train=None, epochs=10):
        ...
        """
        logger.info(f"   ↳ [LSTM] Gerando sequências (Gap Max: {self.max_gap_seconds}s)...")
        X_seq_all, indices_all = self.create_sequences_with_index()
        if len(X_seq_all) == 0:
            logger.warning(
                f"Nenhuma sequência válida encontrada para {strategy_name} (Verifique gaps)."
            )
        ...
        if mask_train is not None:
            ...
            if len(X_train) == 0:
                logger.warning("Treino vazio após filtro de máscara.")
                return None, None, None
        else:
            X_train = X_seq_all
        # Modelo e Treino
        ...
        model = keras.Sequential([
            keras.layers.LSTM(32, activation='relu', input_shape=(self.window_size, n_features), return_sequences=True),
        ...
        es = keras.callbacks.EarlyStopping(monitor='loss', patience=3, mode='min', restore_best_weights=True)
        logger.info(f"   ↳ Treinando {strategy_name} com {len(X_train)} sequências...")
